...
![](images/direct-vs_transitive_access.png){width=450}

::: {.callout-note appearance="minimal"}
**Direct vs. transitive access.** A point-in-time report confirms that `jsmith` holds no sensitive direct group memberships. The graph reveals a two-hop path from `Finance-Users` to `Domain-Admins` through nested group relationships invisible to the report. The spreadsheet answer and the graph answer are both technically accurate and point to opposite risk conclusions.
:::
